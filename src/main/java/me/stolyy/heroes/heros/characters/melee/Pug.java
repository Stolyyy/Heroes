...
import me.stolyy.heroes.heros.characters.minions.Basher;
import me.stolyy.heroes.heros.characters.minions.Princess;
...
public class Pug extends Hero implements UseSneak {
    ...
    public Pug(Player player, HeroConfig config) {
        super(player, config);
    }
    ...
    private Ability primary(){
        return new Dash(this, config.abilityConfigs().get("primary"));
    }

    private Ability secondary(){
        return new Ability(this, config.abilityConfigs().get("secondary")) {
            @Override
            public void onUse() {
                ...
                int ticks = (int) config.getProperty("ticks");
                double spacing = (double) config.getProperty("spacing");
    ...
    private Ability ultimate() {
        Hero h = this;
        return new Ability(this, config.abilityConfigs().get("ultimate")) {
            Basher basher;
            Princess princess;
            ...
            public void onUse() {
                ...
                Heroes.getScheduler().runTaskLater(Heroes.getInstance(), this::onEnd, (long) config.duration() * 20);
            }
            @Override
            public void onEnd() {
                if (basher != null) {
    ...
    private Ability passive() {
        return new Ability(this, config.abilityConfigs().get("passive")) {
            BukkitTask pounce;
            ...
            public double energyCost(){
                return (double) config.getProperty("cost");
            }
